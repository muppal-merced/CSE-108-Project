...
class Lobby(db.Model):
    ...
    player2 = db.relationship(
        'User',
        backref='joined_lobbies',
        foreign_keys='Lobby.player2_id'
    )

    # READY SYSTEM
    creator_ready = db.Column(
        db.Boolean,
        default=False
    )

    player2_ready = db.Column(
        db.Boolean,
        default=False
    )
...
class Game(db.Model):
    ...
    secret_word = db.Column(
        db.String(10),
        nullable=False
    )

    player1_guesses = db.Column(
        db.Text,
        default=''
    )

    player2_guesses = db.Column(
        db.Text,
        default=''
    )

    player1_solved = db.Column(
        db.Boolean,
        default=False
    )

    player2_solved = db.Column(
        db.Boolean,
        default=False
    )
